...
import java.io.*;
import java.util.*;
import mytools.UniformPoint;
import spea2.GlobData;


public class populations {
    private int N = 100;//种群的大小
    private int maxFE = 200;//迭代次数
    private int varNum;//个体决策变量数
    private int objNum;//目标个数
    ...
    private String testFunName;

    public populations(String tFN, int oNum, int vNum) { //构造函数
        testFunName = tFN;
        objNum = oNum;
        varNum = vNum;
        V = new UniformPoint();
    ...
    public void run() {
        ...
        for (int i=0;i<N;i++){      //输出最终每个个体的目标空间结果
            ...
            dataStr += pop[i].obj[0]+"\t"+pop[i].obj[1] + "\n";
        }

        try {
            File file = new File("E:\\algorithmTest\\res\\tadfPF_"+ testFunName +"obj"+ objNum + "var" + varNum +".txt");
            if(!file.exists()) {
                file.createNewFile();
            }
    ...
    public double calIGD(double[][] pf){
        ...
        for (int i = 0 ; i < samplePointNum; i ++){
            ...
            for(int j = 0 ; j < this.N; j ++){
                double dis = 0;
                for (int k = 0; k < this.objNum ; k++){
                    dis += Math.pow(pop[j].obj[k] - pf[i][k],2);
                }
                dis = Math.sqrt(dis);
                if (dis < minDis){
                    minDis = dis;
    ...
    public void relese(individual[] pop){
        for (int i=0;i<pop.length;i++){
                ...
                pop[i].rj = -1;
                pop[i].S = 0;
                pop[i].n_var = new double[N];//暂定为有100个决策变量
                pop[i].number = 0;
            pop[i].calculate_MMF1(testFunName);//计算该个体的目标空间值
        }
    }
    ...
    private void init() {
        ...
        pop[0] = new individual(0, testFunName, N, objNum, varNum);//记录下第一个个体目标空间值，方便后面比较出最小的
        double a = pop[0].obj[0];
        double b = pop[0].obj[1];

        for (int i = 1; i < N; i++) {
            pop[i] = new individual(i, testFunName, N, objNum, varNum);//调用构造函数，对种群中的每一个个体进行初始化操作 产生一个随机点,并计算该点对应目标空间的值
            //找到目标空间的一个最小向量作为参考点Z
            if (pop[i].obj[0] < a) {
                a = pop[i].obj[0];
            }
    ...
    private individual[] UpdateCA(individual[] CAx, individual[] popx, int NCAx, double[][] Rx, double[] Zx) {
        ...
        for (int i=0;i<Rx.length;i++){
            if(w_count[i]!=0){
                ...
                for (int j=0;j<w_count[i] - 1;j++){//更新W_I
                    W_I[i][j]=W_I[i][j+1];
                }
                w_count[i]--;
                c_count[i]++;
    ...
    public void indator_C(individual[] Popx) {
        ...
        crowding a=new crowding(varNum);
    ...
    public individual[] UpdateDA(individual[] DAx, individual[] popx,individual[] CAx,int NDAx, double[][] Rx, double[] Zx){
        ...
        //计算Dec拥挤度
        crowding b=new crowding(varNum);
        b.crowding(Pop);
