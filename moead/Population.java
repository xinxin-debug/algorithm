...
public class Population {
    ...
    public void initPop() {
        for (int i = 0; i < moead.GlobData.popsize; i++) {
            for (int j = 0; j < moead.GlobData.nvar; j++) {
                double rd = Math.random();
                //ind[i].xreal[j] = GlobData.lim[j][0];
                ind[i].xreal[j] = rd * (moead.GlobData.lim[j][1] - moead.GlobData.lim[j][0]) + moead.GlobData.lim[j][0];
            }
        }
    }
    ...
    public void func(int fun_input) {
        ...
        double sum, g;
        double[] x = new double[moead.GlobData.nvar];
        double[] f = new double[moead.GlobData.nfunc];

        // 测试函数 ————ZDT1
        for (int i = 0; i < moead.GlobData.popsize; i++) {
            System.arraycopy(ind[i].xreal, 0, x, 0, moead.GlobData.nvar);
            ...
            // 更新个体的函数值
            System.arraycopy(f, 0, ind[i].fitness, 0, moead.GlobData.nfunc);
        }
    }

    // 计算当前种群的最优P值 Z
    public void calBestZ() {
        // 初始化 Z 为种群第一个个体的函数值
        System.arraycopy(ind[0].fitness, 0, GlobData.Z, 0, GlobData.nfunc);

        // 计算 Z[i] 为种群最小的第 i 个目标函数值
        for (int i = 0; i < GlobData.nfunc; i++) {
            for (int j = 0; j < GlobData.popsize; j++) {
                if (ind[j].fitness[i] < GlobData.Z[i])
                    GlobData.Z[i] = ind[j].fitness[i];
            }
        }
    }
}
